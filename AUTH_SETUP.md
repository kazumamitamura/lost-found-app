# 認証機能のセットアップガイド

このドキュメントでは、忘れ物管理システムの認証機能のセットアップ方法を説明します。

## このアプリで使う Supabase のテーブル

| 用途           | テーブル名           | 説明 |
|----------------|----------------------|------|
| **登録者（管理者）** | **`lost_registrants`** | このアプリに登録した人（教員・職員など）を管理。ここにメールが登録されていて、かつ Supabase Auth で同じメールでログインしている人だけが管理画面にアクセスできます。 |
| 忘れ物データ   | `lost_items`         | 忘れ物の一覧・返却状態など。 |
| 画像           | Storage バケット `lf-images` | 忘れ物の写真。 |

接頭語 `lost_` により、他のアプリ（同じ Supabase プロジェクト）のテーブルと混同しません。

### 閲覧制限・管理者登録キー

- **閲覧**: 忘れ物一覧・詳細・返却ページは、学校メール（`@haguroko.ed.jp`）を入力した人だけが閲覧できます。リンクを知っていても、このメールを入力しないと一覧が見えません。設定は `lib/viewer.ts` の `VIEWER_ALLOWED_EMAIL_DOMAIN` で変更できます。
- **管理者登録**: 新規登録画面では「管理者登録用パスワード」として **8965** を入力した人だけが登録できます。キーは環境変数 `SIGNUP_SECRET` で変更可能（未設定時は 8965）。Vercel の環境変数に `SIGNUP_SECRET=任意の文字列` を設定すると、その文字列が登録キーになります。

## 概要

管理画面（登録、管理ダッシュボード、登録者管理）へのアクセスは、事前に登録された管理者のみに制限されています。

## 1. Supabase Authの設定

### 1-1. Email認証の有効化

1. Supabaseダッシュボードを開く
2. **Authentication** → **Providers** を選択
3. **Email** プロバイダーが有効になっていることを確認
4. **Confirm email** を **無効** にする（学校内での使用を想定）
   - または有効のままにして、メール確認を必須にすることもできます

### 1-2. セキュリティ設定

1. **Authentication** → **Policies** を選択
2. `lost_registrants` テーブルのRLSポリシーが正しく設定されていることを確認

## 2. 初回管理者アカウントの作成

### 方法1: Supabaseダッシュボードから作成（推奨）

1. **Authentication** → **Users** を選択
2. **Add user** ボタンをクリック
3. 以下を入力：
   - **Email**: 管理者のメールアドレス
   - **Password**: 安全なパスワード（8文字以上推奨）
   - **Auto Confirm User**: ✅ チェックを入れる
4. **Create user** をクリック

5. 次に、**登録者テーブル** `lost_registrants` にも同じメールアドレスを登録：
   - **Database** → **Table Editor** → `lost_registrants` を選択
   - **Insert row** をクリック
   - **name**: 管理者名
   - **email**: Authで作成したのと同じメールアドレス
   - **role**: 教員（または職員・その他）
   - **Save** をクリック

### 方法2: SQLで一括作成

以下のSQLを実行して、複数の管理者を一度に設定できます：

```sql
-- 1. lost_registrants テーブルに管理者を追加（role は '教員'/'職員'/'その他' のいずれか）
INSERT INTO lost_registrants (name, email, role, notes)
VALUES
  ('山田太郎', 'yamada@school.com', '教員', '管理者'),
  ('佐藤花子', 'sato@school.com', '教員', '管理者'),
  ('鈴木次郎', 'suzuki@school.com', '教員', '管理者');

-- 2. Supabase Authのユーザーは、Supabaseダッシュボードから手動で作成するか、
--    次のステップで初回ログイン時に自動作成されるように設定します
```

## 3. 自動ユーザー登録の設定（オプション）

初回ログイン時に自動的にAuthユーザーを作成する場合は、以下の機能を追加できます。

### 管理者による招待リンク生成機能

`app/admin/registrants/page.tsx` に以下の機能を追加：

1. 登録者の追加時に、Supabase Authのユーザーも自動作成
2. 初回ログイン用の招待メールを送信

コード例：

```typescript
// 登録者追加時にAuthユーザーも作成
const { data: authUser, error: authError } = await supabase.auth.admin.createUser({
  email: newEmail,
  password: temporaryPassword, // 一時パスワード
  email_confirm: true, // メール確認をスキップ
});

// または、招待メールを送信
const { data, error } = await supabase.auth.admin.inviteUserByEmail(newEmail);
```

## 4. ログインフロー

### ユーザー側のフロー

1. ユーザーが「📝 登録」または「📊 管理」ボタンをクリック
2. 自動的に `/admin/login` ページにリダイレクト
3. メールアドレスとパスワードを入力してログイン
4. 認証成功後、元のページにアクセス

### 認証チェックの流れ

```
ユーザーが管理ページにアクセス
    ↓
ProtectedRoute コンポーネントが認証状態をチェック
    ↓
┌─ 認証済み → ページを表示
└─ 未認証 → /admin/login にリダイレクト
    ↓
ログイン成功
    ↓
lost_registrants テーブルで登録者として存在するか確認
    ↓
┌─ 登録者 → ダッシュボードにリダイレクト
└─ 未登録 → エラーメッセージ表示
```

## 5. パスワードリセット（オプション）

パスワードを忘れた場合の機能を追加する場合：

### 5-1. Supabase設定

1. **Authentication** → **Email Templates** を選択
2. **Reset Password** テンプレートを確認・カスタマイズ

### 5-2. パスワードリセット画面の追加

`app/admin/forgot-password/page.tsx` を作成：

```typescript
const handleResetPassword = async (email: string) => {
  const { error } = await supabase.auth.resetPasswordForEmail(email, {
    redirectTo: `${window.location.origin}/admin/reset-password`,
  });
  
  if (error) {
    // エラー処理
  } else {
    // 成功メッセージ
  }
};
```

## 6. セキュリティのベストプラクティス

### 6-1. パスワードポリシー

- 最低8文字以上
- 大文字・小文字・数字を含む
- 定期的な変更を推奨

### 6-2. RLSポリシーの確認

以下のSQLで、適切なRLSポリシーが設定されているか確認：

```sql
-- lost_registrants テーブルのポリシー確認
SELECT * FROM pg_policies WHERE tablename = 'lost_registrants';

-- 必要に応じて、認証済みユーザーのみがアクセスできるように変更
DROP POLICY IF EXISTS lost_registrants_select_public ON lost_registrants;
DROP POLICY IF EXISTS lost_registrants_insert_public ON lost_registrants;
DROP POLICY IF EXISTS lost_registrants_update_public ON lost_registrants;
DROP POLICY IF EXISTS lost_registrants_delete_public ON lost_registrants;

-- 認証済みユーザーのみがアクセス可能にする
CREATE POLICY lost_registrants_select_authenticated ON lost_registrants
  FOR SELECT TO authenticated USING (true);

CREATE POLICY lost_registrants_insert_authenticated ON lost_registrants
  FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY lost_registrants_update_authenticated ON lost_registrants
  FOR UPDATE TO authenticated USING (true);

CREATE POLICY lost_registrants_delete_authenticated ON lost_registrants
  FOR DELETE TO authenticated USING (true);
```

## 7. トラブルシューティング

### ログインできない

1. **メールアドレスが登録されていない**
   - Supabase Dashboard → Database → lost_registrants テーブルを確認
   - メールアドレスが登録されているか確認

2. **Authユーザーが作成されていない**
   - Supabase Dashboard → Authentication → Users を確認
   - ユーザーが存在するか確認

3. **パスワードが間違っている**
   - Supabase Dashboard → Authentication → Users でユーザーを選択
   - **Reset password** でパスワードをリセット

### ログイン後すぐにログアウトされる

- ブラウザのCookieが無効になっている可能性があります
- ブラウザの設定でCookieを有効にしてください

### 「登録されていないメールアドレスです」と表示される

- `lost_registrants` テーブルにメールアドレスが登録されていません
- 管理者に連絡して登録してもらってください

## 8. 初回デプロイ後のチェックリスト

- [ ] Supabase Authが有効になっている
- [ ] 初回管理者アカウントが作成されている（Auth + lost_registrants）
- [ ] ログインページ（/admin/login）にアクセスできる
- [ ] ログイン後、管理画面にアクセスできる
- [ ] ログアウトボタンが表示され、機能する
- [ ] 未認証状態で管理画面にアクセスすると、ログインページにリダイレクトされる

## まとめ

認証機能により、以下が実現されます：

- ✅ 管理画面へのアクセス制限
- ✅ 事前登録された管理者のみが操作可能
- ✅ セキュアなログイン・ログアウト機能
- ✅ 検索ページは誰でもアクセス可能（生徒用）

何か問題がある場合は、このドキュメントを参照してトラブルシューティングを行ってください。
